...
//Q: 通过方法来访问字段和直接访问字段有什么区别？
//A: 对象与他们能力/行为有关，而字段是对象实现行为/能力的内部细节，这些细节不应该被暴露在外，即使有时候直接调用和通过方法调用字段的结果差别不大。
  //C#是一门面向对象编程语言，我们在写C#程序的时候应该遵循OOP准则，即对象应该对外显示它们能够做什么而不是怎么做。如在VS中，当我们敲打出一个类的实例，
  //我们希望VS自动提示列表中显示的是关于这个实例能干什么的一目了然的方法，而不是一大堆看不明白不知道有什么用的奇怪字段/变量。这点非常非常非常重要。
  
//暂时想到这么多，最后编辑2017/11/27 
