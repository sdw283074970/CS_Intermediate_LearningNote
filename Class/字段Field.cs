...
//然而一些程序员也不喜欢这样，觉得到处都初始化或搞得代码很乱，他们宁愿在构造器中统一初始化。这完全是个人爱好。

//Q: 什么是只读类型字段？
//A: 修饰符只读(readonly)保证了被修饰的字段只会被赋值一次，而后不可更改。

//Q: 为什么要有只读修饰符？为什么我们需要用到这个？
//A: 为了保证数据安全。如在其他方法中可能需要调用到这个字段，如果我们不想其内容被覆盖/丢失，那么我们就需要用到readonly修饰符，它会通过报错提醒程序员
  //不要在其他地方重新初始化这个受只读保护的字段。
